...
// POST /login - Avec validation et protection
router.post('/login', redirectIfAuthenticated, [
  ...
  try {
    ...
    // (même si l'utilisateur n'existe pas, on compare quand même)
    const DUMMY_HASH = '$2a$12$dummyhashfortimingprotection000000000000000000000000000';
    ...
    if (!result.rows[0] || !passwordMatch || !result.rows[0].is_active) {
      ...
      // Message volontairement vague (ne pas indiquer si c'est l'user ou le mdp)
      return res.render('login', {
        title: 'Connexion',
